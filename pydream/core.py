...
def run_dream(parameters, likelihood, nchains=5, niterations=50000, start=None, restart=False, verbose=True,
    ...
    if tempering:
        ...
        sampled_params, log_ps = _sample_dream_pt(nchains, niterations, step_instance, start, pool,
                                                  verbose=verbose)
        ...
        dream_inits = [step_instance] * nchains
        sim_partial = partial(
            _run_dream,
            iterations=niterations, verbose=verbose, nverbose=nverbose
        )
        results = [pool.submit(sim_partial, *arg)
                   for arg in zip(dream_inits, start)]
        try:
...
def _run_dream(dream_instance, start, iterations, verbose, nverbose):
    try:
        ...
        for iteration in range(iterations):
            if iteration % nverbose == 0:
                ...
                if True:
                    print(f'{name} Iteration: {iteration} acceptance rate: {acceptance_rate}')
                    if iteration % 100 == 0 and iteration:
                        accept_rate_100 = float(naccepts100win) / 100
                        print(f'Iteration: {iteration} acceptance rate over last 100 iterations: {accept_rate_100}')
                        naccepts100win = 0
            old_params = q0
...
def _sample_dream_pt(nchains, niterations, step_instance, start, pool, verbose):
    ...
    for iteration in range(niterations):
        ...
        if iteration % 10 == 0:
            ...
            if verbose:
                print('Iteration: ', iteration, ' overall acceptance rate: ', acceptance_rate,
                      ' temp swap acceptance rate per chain: ', Tacceptance_rate,
                      ' and overall temp swap acceptance rate: ', overall_Tacceptance_rate)
            if iteration % 100 == 0:
                ...
                if verbose:
                    print('Iteration: ', iteration, ' overall acceptance rate over last 100 iterations: ',
                          acceptance_rate_100win, ' temp swap acceptance rate: ', Tacceptance_rate_100win,
                          ' and overall temp swap acceptance rate: ', overall_Tacceptance_rate_100win)
                naccepts100win = np.zeros((nchains))
                nacceptsT100win = np.zeros((nchains))
        ...
        if np.log(np.random.uniform()) < alpha:
            if verbose:
                print('Accepted temperature swap of chains: ', random_chains, ' at temperatures: ', T1, ' and ', T2,
                      ' and logps: ', logp1, ' and ', logp2)
            ...
            if verbose:
                print('Did not accept temperature swap of chains: ', random_chains, ' at temperatures: ', T1, ' and ',
                      T2, ' and logps: ', logp1, ' and ', logp2)
...
def _setup_mp_dream_pool(nchains, niterations, step_instance, start_pt=None, mp_context=None):
    ...
    if step_instance.history_file != False:
        ...
        if niterations < step_instance.history_thin:
            arr_dim = ((np.floor(
                nchains * niterations / step_instance.history_thin) + nchains) * step_instance.total_var_dimension) + len_old_history
        else:
            arr_dim = np.floor((((
                                         nchains * niterations) * step_instance.total_var_dimension) / step_instance.history_thin)) + len_old_history
    else:
        if niterations < step_instance.history_thin:
            arr_dim = ((np.floor(
                nchains * niterations / step_instance.history_thin) + nchains) * step_instance.total_var_dimension) + (
                              step_instance.nseedchains * step_instance.total_var_dimension)
        else:
            arr_dim = np.floor(
                ((nchains * niterations / step_instance.history_thin) * step_instance.total_var_dimension)) + (
                              step_instance.nseedchains * step_instance.total_var_dimension)

    min_nseedchains = 2 * len(step_instance.DEpairs) * nchains

    if step_instance.nseedchains < min_nseedchains:
        raise Exception('The size of the seeded starting history is insufficient.  Increase nseedchains>=%s.' % str(
            min_nseedchains))
    ...
    if start_pt is not None:
        if step_instance.start_random:
            print('Warning: start position provided but random_start set to True.  '
...
def _mp_dream_init(arr, cp_arr, nchains, crossover_probs, ncrossover_updates, delta_m, gamma_probs, ngamma_updates,
                   delta_m_gamma, val, switch):
