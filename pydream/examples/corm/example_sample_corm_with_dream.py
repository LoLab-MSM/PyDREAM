...
kfs_to_change = ['kf_AA_cat2', 'kf_AA_cat3', 'kf_AG_cat2', 'kf_AG_cat3', 'kf_AA_allo1', 'kf_AA_allo2', 'kf_AA_allo3',
                 'kf_AG_allo1', 'kf_AG_allo2']
...
kd_AA_cat2 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AA_cat2'].value /
                                             cox2_model.parameters['kf_AA_cat2'].value), scale=1.5)
kcat_AA2 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kcat_AA2'].value), scale=.66)
kd_AA_cat3 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AA_cat3'].value /
                                             cox2_model.parameters['kf_AA_cat3'].value), scale=1.5)
kcat_AA3 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kcat_AA1'].value), scale=.66)
kd_AG_cat2 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AG_cat2'].value /
                                             cox2_model.parameters['kf_AG_cat2'].value), scale=1.5)
kd_AG_cat3 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AG_cat3'].value /
                                             cox2_model.parameters['kf_AG_cat3'].value), scale=1.5)
kcat_AG3 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kcat_AG3'].value), scale=.66)
kd_AA_allo1 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AA_allo1'].value /
                                              cox2_model.parameters['kf_AA_allo1'].value), scale=1)
kd_AA_allo2 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AA_allo2'].value /
                                              cox2_model.parameters['kf_AA_allo2'].value), scale=1)
kd_AA_allo3 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AA_allo3'].value /
                                              cox2_model.parameters['kf_AA_allo3'].value), scale=1)
kd_AG_allo1 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AG_allo1'].value /
                                              cox2_model.parameters['kf_AG_allo1'].value), scale=1)
kd_AG_allo2 = SampledParam(norm, loc=np.log10(cox2_model.parameters['kr_AG_allo2'].value /
                                              cox2_model.parameters['kf_AG_allo2'].value), scale=1)

sampled_parameter_names = [kd_AA_cat2, kcat_AA2, kd_AA_cat3, kcat_AA3, kd_AG_cat2, kd_AG_cat3, kcat_AG3, kd_AA_allo1,
                           kd_AA_allo2, kd_AA_allo3, kd_AG_allo1, kd_AG_allo2]
...
if __name__ == '__main__':
    ...
    sampled_params, log_ps = run_dream(parameters=sampled_parameter_names, likelihood=likelihood,
                                       niterations=niterations, nchains=nchains, multitry=False, gamma_levels=4,
                                       adapt_gamma=True, history_thin=1, model_name='corm_dreamzs_5chain', verbose=True)

    # Save sampling output (sampled parameter values and their corresponding logps).
    for chain in range(len(sampled_params)):
        np.save('corm_dreamzs_5chain_sampled_params_chain_' + str(chain)+'_'+str(total_iterations),
    ...
    if np.any(GR > 1.2):
        ...
        while not converged:
            ...
            sampled_params, log_ps = run_dream(parameters=sampled_parameter_names, likelihood=likelihood,
                                               ...
                                               model_name='corm_dreamzs_5chain', verbose=True, restart=True)
    ...
    run_kwargs = {'parameters': sampled_parameter_names, 'likelihood': likelihood, 'niterations': niterations,
                  ...
                  'model_name': 'corm_dreamzs_5chain', 'verbose': False}
