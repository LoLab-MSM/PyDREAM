...
y100_data = norm(loc=y100, scale = 0.10)
y10_data = norm(loc = y10, scale = 0.10)
...
if __name__ == '__main__':
    ...
    for chain in range(len(sampled_params)):
        np.save('necro_smallest_dreamzs_5chain_sampled_params_chain21320_' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
        np.save('necro_smallest_dreamzs_5chain21320_logps_chain_' + str(chain)+'_'+str(total_iterations), log_ps[chain])

    #Check convergence and continue sampling if not converged

    GR = Gelman_Rubin(sampled_params)
    print('At iteration: ',total_iterations,' GR = ',GR)
    np.savetxt('necro_smallest_dreamzs21320_5chain_GelmanRubin_iteration_'+str(total_iterations)+'.txt', GR)
    ...
    if np.any(GR>1.2):
        ...
        while not converged:
            ...
            # Save sampling output (sampled parameter values and their corresponding logps).
            for chain in range(len(sampled_params)):
                np.save('necro_smallest_dreamzs21320_5chain_sampled_params_chain_' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
                np.save('necro_smallest_dreamzs21320_5chain_logps_chain_' + str(chain)+'_'+str(total_iterations), log_ps[chain])
    ...
    try:
        ...
        for dim in range(ndims):
            ...
            fig.savefig('PyDREAM_necro21320_smallest_dimension_'+str(dim))
