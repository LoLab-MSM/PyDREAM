...
sd100 = np.array([0.0001, 0.001, 0.001, 0.05, 0.05, 0.20, 0.05, 0.05, 0.05])
sd10 = np.array([0.0001, 0.001, 0.001, 0.05, 0.05, 0.05, 0.05, 0.10, 0.05])
...
#run_pydream(..., mp_context=get_context('spawn'))

converged = False
sampled_params, log_ps = run_dream(mp_context=get_context('spawn'),parameters=sampled_params_list,
                                   ...
                                   adapt_gamma=True,
                                   history_thin=1,
                                   model_name='dreamzs_5chain_22620_necro',
                                   verbose=True)

total_iterations = niterations
# Save sampling output (sampled parameter values and their corresponding logps).
for chain in range(len(sampled_params)):
    np.save('dreamzs_5chain_sampled_params_chain_22620_' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
    np.save('dreamzs_5chain_logps_chain_22620_' + str(chain)+'_'+str(total_iterations), log_ps[chain])
GR = Gelman_Rubin(sampled_params)
print('At iteration: ',total_iterations,' GR = ',GR)
np.savetxt('dreamzs_5chain_GelmanRubin_iteration_22620_'+str(total_iterations)+'.txt', GR)
...
if np.any(GR>1.2):
    ...
    try:
        ...
        for dim in range(ndims):
            ...
            fig.savefig('fig_PyDREAM_dimension_22620_'+str(dim))
    ...
    run_kwargs = {'parameters':sampled_params_list, 'likelihood':likelihood, 'niterations':niterations, 'nchains':nchains, \
                  'multitry':False, 'gamma_levels':4, 'adapt_gamma':True, 'history_thin':1,    'model_name':'dreamzs_5chain_22620_necro', 'verbose':False}  
