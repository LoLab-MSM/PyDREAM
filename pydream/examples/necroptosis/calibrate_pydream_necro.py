...
if __name__ == '__main__':
    ...
    sampled_params, log_ps = run_dream(parameters=sampled_params_list, likelihood=likelihood,
                                       ...
                                       model_name='necro_smallest_dreamzs116_5chainnew', verbose=True)

    # Save sampling output (sampled parameter values and their corresponding logps).
    for chain in range(len(sampled_params)):
        np.save('necro_smallest_dreamzs_5chain_sampled_params_chain116_new' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
        np.save('necro_smallest_dreamzs_5chain_logps_chain116_new' + str(chain)+'_'+str(total_iterations), log_ps[chain])

    #Check convergence and continue sampling if not converged

    GR = Gelman_Rubin(sampled_params)
    print('At iteration: ',total_iterations,' GR = ',GR)
    np.savetxt('necro_smallest_dreamzs_5chain116_GelmanRubin_iteration_new'+str(total_iterations)+'.txt', GR)
    ...
    if np.any(GR>1.2):
        ...
        while not converged:
            ...
            sampled_params, log_ps = run_dream(parameters=sampled_params_list, likelihood=likelihood,
                                               ...
                                               adapt_gamma=True, history_thin=1, model_name='necro_smallest_dreamzs116_5chainnew',
                                               verbose=True, restart=True)


            # Save sampling output (sampled parameter values and their corresponding logps).
            for chain in range(len(sampled_params)):
                np.save('necro_smallest_dreamzs116_5chain_sampled_params_chainnew_' + str(chain)+'_'+str(total_iterations), sampled_params[chain])
                np.save('necro_smallest_dreamzs116_5chain_logps_chainnew_' + str(chain)+'_'+str(total_iterations), log_ps[chain])

            old_samples = [np.concatenate((old_samples[chain], sampled_params[chain])) for chain in range(nchains)]
            GR = Gelman_Rubin(old_samples)
            print('At iteration: ',total_iterations,' GR = ',GR)
            np.savetxt('necro_smallest_dreamzs116_5chain_GelmanRubin_iterationnew_' + str(total_iterations)+'.txt', GR)
    ...
    try:
        ...
        for dim in range(ndims):
            ...
            fig.savefig('PyDREAM_necro116_smallest_dimensionnew_'+str(dim))
    ...
    run_kwargs = {'parameters':sampled_params_list, 'likelihood':likelihood, 'niterations':niterations, 'nchains':nchains, \
                  'multitry':False, 'gamma_levels':4, 'adapt_gamma':True, 'history_thin':1, 'model_name':'necro_smallest_dreamzs116_5chainnew', 'verbose':False}
